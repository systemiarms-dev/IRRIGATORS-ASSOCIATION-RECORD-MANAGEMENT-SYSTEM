                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-slate-600 font-extrabold flex items-center gap-1.5 mt-0.5">
                    ...
                    <span className="font-mono text-emerald-800 bg-emerald-50 px-2 py-0.5 rounded border border-emerald-200">
                      {profile?.email.includes('@iarms.org') ? profile.email.split('@')[0] : profile?.email}
                    </span>
                  </div>
        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 text-white shadow-md space-y-3">
            ...
            <ul className="text-xs text-slate-300 space-y-1.5 list-disc list-inside font-medium">
              {profile?.role === 'admin' && (
                <>
                  <li>Full administrative user account management & role assignment.</li>
                  <li>Password reset authority for all registered members.</li>
                  <li>Complete financial ledger inspection & report generation.</li>
                </>
              )}
              {profile?.role === 'treasurer' && (
                <>
                  <li>Logging member ISF collections & official receipt vouchers.</li>
                  <li>Recording operational disbursements and budget categories.</li>
                  <li>Exporting financial reports and summary statements.</li>
                </>
              )}
              {profile?.role === 'auditor' && (
                <>
                  <li>Reviewing pending expense vouchers & receipt uploads.</li>
                  <li>Approving or rejecting treasurer disbursement logs.</li>
                  <li>Inspecting NIA standard financial statement balance sheets.</li>
                </>
              )}
              {(profile?.role as string) === 'president' && (
                <>
                  <li>Executive overview of association cash flow trends & summaries.</li>
                  <li>Reviewing NIA standard financial statements (FS1 - FS4).</li>
                  <li>Monitoring system audit logs and member activity.</li>
                </>
              )}
              {(profile?.role as string) === 'collector' && (
                <>
                  <li>Collecting field Irrigation Service Fees (ISF).</li>
                  <li>Submitting payment logs to the Association Treasurer.</li>
                </>
              )}
              {profile?.role === 'member' && (
                <>
                  <li>Viewing personal ISF fee payment history and statements.</li>
                  <li>Updating personal contact details and profile preferences.</li>
                </>
              )}
            </ul>
          </div>
