...
export function getRoleBadgeProps(role: string): { label: string; variant: string } {
  switch (role?.toLowerCase()) {
    case 'admin':
      return { label: 'Administrator', variant: 'rose' };
    case 'treasurer':
      return { label: 'Treasurer', variant: 'emerald' };
    case 'auditor':
      return { label: 'Auditor', variant: 'indigo' };
    case 'president':
      return { label: 'Association President', variant: 'purple' };
    case 'collector':
      return { label: 'Field Collector', variant: 'blue' };
    default:
      return { label: 'Association Member', variant: 'amber' };
  }
}
...
export function getReceiptImageUrl(receipt: any): string {
  ...
  const uploaderName = receipt?.uploader?.full_name || 'RIC UNDAY';
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="600" height="750" viewBox="0 0 600 750">
    ...
    <text x="300" y="55" font-family="Arial, sans-serif" font-size="17" font-weight="900" fill="#ffffff" text-anchor="middle">NANGURISAN LAYA FARMERS IRRIGATORS ASSOC.</text>
    <text x="300" y="78" font-family="Arial, sans-serif" font-size="12" font-weight="700" fill="#04B358" text-anchor="middle">IARMS EXECUTIVE OFFICIAL CASH VOUCHER / RECEIPT</text>
    <text x="300" y="100" font-family="Arial, sans-serif" font-size="11" font-weight="400" fill="#e2e8f0" text-anchor="middle">Gonzaga, Cagayan • IA Reg. No. 2026-IARMS-NLFIA</text>
